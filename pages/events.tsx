...
const EventsPage = (props: any) => {
  ...
  function mintOnChain(eventInfo: EventWithTickets) {
    const { title, location, startDate, endDate, category } = eventInfo;
    let metaData = JSON.stringify({
      ...
      pinataContent: {
        //this is the metadata being stored
        event: title,
        location: location,
        startDate: startDate,
        endDate: endDate,
  ...
  async function mintTicket() {
    ...
    const { scAddress, ticketCount, ticketURIs } = eventInfo;

    let ticketSupply = ticketCount;
    ...
    response = await mintOnChain(eventInfo);
    let ipfsHash = response.data.IpfsHash;
    // let ipfshash =
    //   "bafkreigqp37qsksqgrf42snchf25hbh22teak2guw5wnlxismbwotipdcq";
    ...
    const mint_ticket = await event_contract.mint("General Admission", link, {
      gasLimit: 2100000,
      value: price_needed,
    });
    /*
    ...
    response = await axios.post("http://localhost:3000/api/events/3", event);
    let data = response.data;
    console.log(data);
  }
  ...
  async function updateEvent() {
    ...
    for (let i = 0; i < ticketURIs.length; i++) {
      ...
      let ipfshash =
        "bafkreihf6l4756g5zdz5czqt7qddeqrecayh5mlduv6w7tdciq6u34q6di";
      console.log(ipfshash);
      const link = "https://gateway.pinata.cloud/ipfs/" + ipfshash;
      console.log("IPFS Hash Link  : ", link);
