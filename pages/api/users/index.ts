import { extractFileExtension } from "./../../../lib/supabase";
    ...
    try {
      ...
      if (profilePic) {
        const { data, error } = await supabase.storage
          .from("user-profile")
          .upload(uuidv4(), await dataUrlToFile(profilePic), {
            contentType: `image/${extractFileExtension(profilePic)}`,
          });
      ...
      if (bannerPic) {
        const { data, error } = await supabase.storage
          .from("user-profile")
          .upload(uuidv4(), await dataUrlToFile(bannerPic), {
            contentType: `image/${extractFileExtension(bannerPic)}`,
          });
